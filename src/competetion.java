...
public class competetion {
	public static void main(String[] args) throws FileNotFoundException{
		...
		double timeLeft = Double.parseDouble(in.next());
		...
		int depth = depthDecision(dimension,boardState,timeLeft);
	...
	public static int depthDecision(int dimension,String[][] boardState,double timeLeft){
		int maxDepth = maxDepthCount(boardState);
		
		if(dimension <= 4){
			return maxDepth;
		}
		else if(dimension == 5 && timeLeft > 120){
			return 5;
		}
		else if(dimension == 5 && timeLeft <= 120 && timeLeft > 10){ 
			return 4;
		}
		else if(dimension == 5 && timeLeft <= 10){
			return 3;
		}
		else if(dimension == 6 && timeLeft > 40){
			return 4;
		}
		else if(dimension == 6 && timeLeft <=40){
			return 3;
	...
	public static int maxDepthCount(String[][] boardState){
		int buffer = 0;
	...
	public static void competeting(Board board) throws FileNotFoundException{
		Operation result = competeingDecision(board);
		outPut(board,result);
	}
	
	public static Operation competeingDecision(Board board){
		Queue<Operation> operationSet = getSuccessor(board);
		HashMap<Operation,Integer> valueSet = new HashMap<Operation,Integer>();
		int a = -999999;
		int b = 999999;
		...
		for(Operation op:operationSet){
			valueSet.put(op, cpminValue(move(board,op),a,b));
			//System.out.println(abminValue(move(board,op),a,b));
			//printMatrix(move(board,op).boardState);
			//System.out.println("-------------");
		}
	...
	public static int cpmaxValue(Board board,int a,int b) {
		Queue<Operation> operationSet = getSuccessor(board);
		if(cutOffCheck(board) || terminalCheck(board)){
			return scoreCount(board);
		}
		int v = -999999;
		for(Operation op : operationSet){
			v =  Math.max(v,cpminValue(move(board,op),a,b));
			if(v >= b) return v;
			a = Math.max(a, v);
		}
		return v;
	...
	public static int cpminValue(Board board,int a, int b) {
		Queue<Operation> operationSet = getSuccessor(board);
		if(cutOffCheck(board) || terminalCheck(board)){
			return scoreCount(board);
		}
		int v = 999999;
		for(Operation op : operationSet){
			v =  Math.min(v,cpmaxValue(move(board,op),a,b));
			if(v <= a) return v;
			b = Math.min(b, v);
		}
		return v;
